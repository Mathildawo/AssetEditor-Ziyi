...
namespace Editors.AnimationVisualEditors.AnimationKeyframeEditor
{
    public partial class AnimationKeyframeEditorViewModel : EditorHostBase, IDisposable
    {
        ...
        {
            int min = int.Parse(SelectedFrameAInterpolation.Value);
            int max = int.Parse(SelectedFrameBInterpolation.Value);
            if (max - min < 2)
            {
                MessageBox.Show("there must be at least 1 frame in between frame A and B to apply interpolation across frames", "error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            FirstFrame();
            while (_rider.Player.CurrentFrame != min)
            {
                NextFrame();
            }  
            float step = 1f / (max - min);

            InterpolationValue.Value = 0f;
            
            for (var i = min+1; i < max; i++)
            {
                _rider.Player.CurrentFrame = i;
                InterpolationValue.Value += step;
                _interpolateBetweenPose.ApplySingleFrame();
            }
            _rider.Player.CurrentFrame = max;
            ResetInterpolationTool();
        }
