    ...
    public class MountLinkViewModel : NotifyPropertyChangedImpl
    {
        ...
        private void MountTagSeleted(AnimationBinEntryGenericFormat value)
        {
           if (value != null)
           {
               var file = _pfs.FindFile(value.AnimationFile);
                var animationRef = _skeletonAnimationLookUpHelper.FindAnimationRefFromPackFile(file, value.AnimationFile); 
               _assetViewModelEditor.SetAnimation(_mount, animationRef.AnimationFile);
        ...
        private void RiderTagSelected(AnimationBinEntryGenericFormat value)
        {
           if (value != null)
           {
               ...
               {
                   var animationRef = _skeletonAnimationLookUpHelper.FindAnimationRefFromPackFile(file, value.AnimationFile);
                   _assetViewModelEditor.SetAnimation(_rider, animationRef.AnimationFile);
               }
           }
           _validateAction();
        }
