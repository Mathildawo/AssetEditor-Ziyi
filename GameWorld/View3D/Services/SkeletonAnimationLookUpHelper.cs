...
namespace GameWorld.Core.Services
{
    ...
    {
        void Dispose();
        AnimationReference? FindAnimationRefFromPackFile(PackFile animation,string path);
    ...
    public class SkeletonAnimationLookUpHelper : IDisposable, ISkeletonAnimationLookUpHelper
    {
        ...
        public AnimationReference? FindAnimationRefFromPackFile(PackFile animation, string path = null)
        {
            string fullPath;
            lock (_threadLock)
            {      
                if (path != null)
                {
                    fullPath = path.Replace('/', '\\').ToLower().Trim();
                }
                else
                    fullPath = _packFileService.GetFullPath(animation);
