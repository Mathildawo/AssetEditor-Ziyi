...
namespace Test.Shared.Core.PackFiles
{
    internal class FileSaveServiceTests
    {
        ...
        public void Save_NoExistingFile_DontPromptOnConflict()
        {
            ...
            _uiProvider.Verify(x => x.DisplaySaveDialog(_pfs, It.IsAny<List<string>>(),"test"), Times.Never);
        }
        ...
        public void Save_ExistingFile_PromptOnConflict_NewPath()
        {
            // Arrange
            var saveService = new FileSaveService(_pfs, _uiProvider.Object);
            _uiProvider.Setup(x => x.DisplaySaveDialog(_pfs, It.IsAny<List<string>>(), "test")).Returns(new SaveDialogResult(true, null, "folder\\subfolder3\\File1.test"));
            ...
            _eventHub.Verify(x => x.PublishGlobalEvent(It.IsAny<PackFileContainerFilesAddedEvent>()), Times.Once);
            _uiProvider.Verify(x => x.DisplaySaveDialog(_pfs, It.IsAny<List<string>>(), "test"), Times.Once);
        }
        ...
        public void Save_ExistingFile_PromptOnConflict_ExistingPath()
        {
            // Arrange
            var saveService = new FileSaveService(_pfs, _uiProvider.Object);
            _uiProvider.Setup(x => x.DisplaySaveDialog(_pfs, It.IsAny<List<string>>(), "test")).Returns(new SaveDialogResult(true, null, "folder\\subfolder\\File0.test"));
        ...
        public void Save_ExistingFile_PromptOnConflict_Exit()
        {
            // Arrange
            var saveService = new FileSaveService(_pfs, _uiProvider.Object);
            _uiProvider.Setup(x => x.DisplaySaveDialog(_pfs, It.IsAny<List<string>>(), "test")).Returns(new SaveDialogResult(false, null, null));
